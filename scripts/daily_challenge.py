...
class DailyChallengeSystem:
    ...
    def display_challenge(self, problem: dict[str, Any] | None) -> None:
        ...
        print(
            f"   Test: languages/python/problems/tests/test_{problem['id'].replace('-', '_')}.py"
        )
        ...
        print("   # Run tests")
        print(
            f"   pytest languages/python/problems/tests/test_{problem['id'].replace('-', '_')}.py"
        )
        print()
        print("   # Mark as complete when done")
        print(
            f"   python scripts/daily_challenge.py --complete {problem['id']} --success"
        )
        print()
    ...
    def show_history(self, days: int = 7) -> None:
        ...
        recent = [
            c
            for c in self.challenge_data["history"]
            if (datetime.now().date() - datetime.fromisoformat(c["date"]).date()).days
            < days
        ]
        ...
        success = sum(
            1 for c in recent if c.get("completed") and c.get("success", True)
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--topic", "-t", type=str, help="Filter by topic (e.g., arrays, trees)"
    )
    parser.add_argument(
        "--complete", "-c", type=str, help="Mark problem as complete (problem ID)"
    )
    parser.add_argument(
        "--success", "-s", action="store_true", help="Mark completion as successful"
    )

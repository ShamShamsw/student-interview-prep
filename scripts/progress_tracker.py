...
class ProgressTracker:
    ...
    def _check_achievements(self) -> None:
        ...
        achievements = [
            (
                ...
                lambda: len(self.progress_data["completed"]) >= 1,
            ),
            (
                "five_solved",
                "Getting Started",
                "Solved 5 problems!",
                lambda: len(self.progress_data["completed"]) >= 5,
            ),
            (
                "ten_solved",
                "Warming Up",
                "Solved 10 problems!",
                lambda: len(self.progress_data["completed"]) >= 10,
            ),
            (
                "twenty_solved",
                "Making Progress",
                "Solved 20 problems!",
                lambda: len(self.progress_data["completed"]) >= 20,
            ),
            (
                "all_solved",
                "Master",
                "Solved all problems!",
                lambda: len(self.progress_data["completed"]) >= len(self.problems),
            ),
            (
                "streak_3",
                "Consistent",
                "3-day streak!",
                lambda: self.progress_data["streak_data"]["current"] >= 3,
            ),
            (
                "streak_7",
                "Dedicated",
                "7-day streak!",
                lambda: self.progress_data["streak_data"]["current"] >= 7,
            ),
            (
                "streak_30",
                "Unstoppable",
                "30-day streak!",
                lambda: self.progress_data["streak_data"]["current"] >= 30,
            ),
        ]
    ...
    def display_progress(self, detailed: bool = False) -> None:
        ...
        for difficulty in ["Easy", "Medium", "Hard"]:
            if difficulty in by_difficulty:
                ...
                print(
                    f"\n  {difficulty:8} {stats['completed']:2}/{stats['total']:2} ",
                    end="",
                )
        ...
        if self.progress_data["attempts"]:
            ...
            for problem_id, attempt in recent:
                date = datetime.fromisoformat(attempt["date"]).strftime(
                    "%Y-%m-%d %H:%M"
                )
        ...
        if len(completed) < total:
            ...
            print("   Run: python scripts/progress_tracker.py --detailed")
        else:
    ...
    def export_progress(self, output_file: str) -> None:
        ...
        export_data = {
            ...
            "completion_rate": (
                len(completed) / len(self.problems) if self.problems else 0
            ),
...
def main() -> None:
    ...
    parser.add_argument(
        "--detailed", "-d", action="store_true", help="Show detailed breakdown by topic"
    )
    ...
    parser.add_argument(
        ...
        "-m",
        type=str,
        help="Mark a problem as complete (problem ID)",
    )

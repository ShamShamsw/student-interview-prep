...
class HintSystem:
    ...
    def _create_default_hints(self) -> dict[str, Any]:
        ...
        return {
            "01-two-sum": {
                "name": "Two Sum",
                "hints": [
                    {"level": 1, "hint": "Think about what data structure allows O(1) lookup time."},
                    {"level": 2, "hint": "For each number, you need to find target - number. Store seen numbers as you iterate."},
                    {"level": 3, "hint": "Use a hash table (dictionary) to store {value: index} pairs."},
                    {"level": 4, "hint": "For each num, check if (target - num) exists in your hash table."},
                    {"level": 5, "hint": "Time: O(n), Space: O(n). One pass through the array."},
                ],
            ...
            "15-valid-palindrome": {
                ...
                "hints": [
                    {"level": 1, "hint": "A palindrome reads the same forwards and backwards."},
                    {"level": 2, "hint": "Use two pointers, one at each end of the string."},
                    {"level": 3, "hint": "Ignore non-alphanumeric characters and compare case-insensitively."},
                    {"level": 4, "hint":"Move pointers inward while they match. If they meet, it's a palindrome."},
                    {"level": 5, "hint": "Time: O(n), Space: O(1). In-place comparison."},
                ],
            ...
            "21-binary-search": {
                ...
                "hints": [
                    ...
                    {"level": 2, "hint": "Each comparison allows you to eliminate half of the remaining elements."},
                    {"level": 3, "hint": "Maintain left and right pointers. Check the middle element."},
                    {"level": 4, "hint": "If target < mid, search left half. If target > mid, search right half."},
                    {"level": 5, "hint": "Time: O(log n), Space: O(1). Classic divide and conquer."},
                ],
            ...
            "26-reverse-linked-list": {
                ...
                "hints": [
                    {"level": 1, "hint": "You need to reverse the direction of all pointers."},
                    {"level": 2, "hint": "Think about reversing one connection at a time."},
                    {"level": 3, "hint": "Keep track of previous, current, and next nodes."},
                    {"level": 4, "hint": "Iterate: save next, point current to previous, advance all pointers."},
            ...
            "35-coin-change": {
                "name": "Coin Change",
                "hints": [
                    {"level": 1, "hint": "This is a classic dynamic programming problem."},
                    {"level": 2, "hint": "Build up solutions for smaller amounts first."},
                    {"level": 3, "hint": "Create a DP array where dp[i] = minimum coins for amount i."},
                    {"level": 4, "hint": "For each amount, try using each coin: dp[i] = min(dp[i], dp[i-coin] + 1)."},
                    {"level": 5, "hint": "Time: O(amount × coins), Space: O(amount). Bottom-up DP."},
                ],
    ...
    def show_hints(
        ...
        if show_all:
            # Show all hints
            for hint in hints:
                print(f"\n{colorize(f'Level {hint[\"level\"]}:', 'yellow')}")
                print(f"   {hint['hint']}")
            ...
            print(colorize("Progressive Hints (use --level N for specific hint):\n", "cyan"))
            print(colorize(f"Level 1 (Concept):", "green"))
...
def main() -> None:
    ...
    parser.add_argument("--list", action="store_true", help="List all problems with hints")

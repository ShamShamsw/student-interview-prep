...
class AchievementSystem:
    ...
    def _define_achievements(self) -> list[Achievement]:
        ...
        current_streak = self.progress_data.get("streak_data", {}).get("current", 0)
        best_streak = self.progress_data.get("streak_data", {}).get("best", 0)
        
        return [
            # Getting Started
            Achievement(
                "first_solve", "First Steps", "Solved your first problem!",
                "🌟", "Bronze", "Getting Started",
                lambda: completed_count >= 1, 10
            ),
            Achievement(
                "five_solved", "Getting Started", "Solved 5 problems!",
                "🔰", "Bronze", "Getting Started",
                lambda: completed_count >= 5, 25
            ),
            Achievement(
                "ten_solved", "Warming Up", "Solved 10 problems!",
                "📈", "Silver", "Getting Started",
                lambda: completed_count >= 10, 50
            ),
            Achievement(
                "twenty_solved", "Making Progress", "Solved 20 problems!",
                "💪", "Silver", "Getting Started",
                lambda: completed_count >= 20, 100
            ),
            Achievement(
                "all_solved", "Master", "Solved all 35 problems!",
                "🏅", "Gold", "Getting Started",
                lambda: completed_count >= 35, 500
            ),
            ...
            Achievement(
                ...
                "🔥", "Bronze", "Streaks",
                lambda: best_streak >= 3, 30
            ),
            Achievement(
                "streak_7", "Dedicated", "Maintain a 7-day streak!",
                "⚡", "Silver", "Streaks",
                lambda: best_streak >= 7, 70
            ),
            Achievement(
                "streak_30", "Unstoppable", "Maintain a 30-day streak!",
                "🚀", "Gold", "Streaks",
                lambda: best_streak >= 30, 300
            ),
            Achievement(
                "streak_100", "Legend", "Maintain a 100-day streak!",
                "💎", "Diamond", "Streaks",
                lambda: best_streak >= 100, 1000
            ),
            
            # Speed Challenges
            Achievement(
                "speed_demon", "Speed Demon", "Solve 5 problems in one day!",
                "⚡", "Silver", "Speed",
                lambda: False,  # TODO: Track this
                100
            ),
            Achievement(
                "marathon", "Marathon Runner", "Solve 10 problems in one week!",
                "🏃", "Gold", "Speed",
                lambda: False,  # TODO: Track this
                200
            ),
    ...
    def display_achievements(self, show_locked: bool = True) -> None:
        ...
        if newly_unlocked:
            ...
            for achievement in newly_unlocked:
                print(f"{achievement.icon} {colorize(achievement.name, 'green')} (+{achievement.points} pts)")
        ...
        for category, achievements in by_category.items():
            ...
            for achievement in achievements:
                ...
                if is_unlocked:
                    print(f"  ✅ {achievement.icon} {colorize(achievement.name, 'green')} [{achievement.tier}]")
                    print(f"      {achievement.description} (+{achievement.points} pts)")
                elif show_locked:
                    print(f"  🔒 ❓ {colorize(achievement.name, 'reset')} [{achievement.tier}]")
                    print(f"      {achievement.description} (+{achievement.points} pts)")
        ...
        print("\n" + "=" * 70)
        print(colorize(f"\n📊 Progress: {unlocked_count}/{total_count} achievements unlocked", "cyan"))
    ...
    def export_badges(self, output_file: str) -> None:
        ...
        lines.append(f"*Earned {len(self.achievement_data['unlocked'])} achievements*\n")
        ...
        for achievement in self.achievements:
            if achievement.id in self.achievement_data["unlocked"]:
                ...
                lines.append(f"**Tier:** {achievement.tier} | **Points:** {achievement.points}\n\n")
...
def main() -> None:
    ...
    parser.add_argument("--next", "-n", action="store_true", help="Show next achievements")
    parser.add_argument("--export", "-e", type=str, help="Export badges to markdown file")
    parser.add_argument("--hide-locked", action="store_true", help="Hide locked achievements")

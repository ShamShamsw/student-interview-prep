...
class CodeAnalyzer:
    ...
    def _analyze_documentation(self, tree: ast.AST, code: str) -> dict[str, Any]:
        ...
        if doc["total_functions"] > 0:
            doc["docstring_coverage"] = (
                doc["function_docstrings"] / doc["total_functions"] * 100
            )
        else:
    ...
    def display_analysis(
        self, analysis: dict[str, Any], detailed: bool = False
    ) -> None:
        """Display analysis results."""
        if "error" in analysis:
            print(
                colorize(
                    f"❌ Error analyzing {analysis['file']}: {analysis['error']}", "red"
                )
            )
            return
        ...
        if detailed:
            ...
            print(
                f"   Module Docstring: {'✅' if doc['has_module_docstring'] else '❌'}"
            )
            print(
                f"   Function Docstrings: {doc['function_docstrings']}/{doc['total_functions']}"
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--all", "-a", action="store_true", help="Analyze all files in directory"
    )
    parser.add_argument(

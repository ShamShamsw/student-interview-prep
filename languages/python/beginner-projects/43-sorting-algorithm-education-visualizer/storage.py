...
"""Persistence layer for Project 43: Sorting Algorithm Education Visualizer."""

DATA_DIR = Path(__file__).resolve().parent / 'data'
OUTPUTS_DIR = DATA_DIR / 'outputs'
ARRAY_LIBRARY_FILE = DATA_DIR / 'array_sets.json'
...
def load_array_library() -> List[Dict[str, Any]]:
    """Load reusable lesson arrays from disk."""
    ensure_data_dirs()
    return _load_list_file(ARRAY_LIBRARY_FILE)


def save_array_library(library: List[Dict[str, Any]]) -> str:
    """Persist lesson arrays to disk."""
    ensure_data_dirs()
    _save_list_file(ARRAY_LIBRARY_FILE, library)
    return str(ARRAY_LIBRARY_FILE)
...
def save_trace_file(trace_payload: Dict[str, Any], session_id: str) -> str:
    """Persist demo trace payload for a session."""
    ensure_data_dirs()
    ...
    catalog.append(
        {
            ...
            'demo_array_label': run_record.get('demo_array_label', ''),
            'demo_array_length': run_record.get('demo_array_length', 0),
            'algorithms_run': run_record.get('algorithms_run', 0),
            'benchmark_points': run_record.get('benchmark_points', 0),
            'elapsed_ms': run_record.get('elapsed_ms', 0.0),
            'fastest_algorithm': run_record.get('metrics', {}).get('fastest_algorithm', ''),

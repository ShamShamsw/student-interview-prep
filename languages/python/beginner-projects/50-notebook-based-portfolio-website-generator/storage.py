...
"""Persistence layer for Project 50: Notebook-Based Portfolio Website Generator."""

DATA_DIR = Path(__file__).resolve().parent / 'data'
OUTPUTS_DIR = DATA_DIR / 'outputs'
PORTFOLIO_LIBRARY_FILE = DATA_DIR / 'portfolio_sets.json'
...
def load_portfolio_library() -> List[Dict[str, Any]]:
    """Load reusable portfolio definitions from disk."""
    ensure_data_dirs()
    return _load_list_file(PORTFOLIO_LIBRARY_FILE)


def save_portfolio_library(library: List[Dict[str, Any]]) -> str:
    """Persist portfolio library to disk."""
    ensure_data_dirs()
    _save_list_file(PORTFOLIO_LIBRARY_FILE, library)
    return str(PORTFOLIO_LIBRARY_FILE)
...
def load_history() -> List[Dict[str, Any]]:
    """Load generator history entries from disk."""
    ensure_data_dirs()
    return _load_list_file(HISTORY_FILE)


def save_history(history: List[Dict[str, Any]]) -> str:
    """Persist generator history entries to disk."""
    ensure_data_dirs()
...
def save_trace_file(trace_payload: Dict[str, Any], session_id: str) -> str:
    """Persist generation trace payload for a session."""
    ensure_data_dirs()
    ...
    catalog.append(
        {
            ...
            'demo_portfolio_label': run_record.get('demo_portfolio_label', ''),
            'strategy_selected': run_record.get('strategy_selected', ''),
            'strategy_runs': run_record.get('strategy_runs', 0),
            'pages_generated': run_record.get('pages_generated', 0),

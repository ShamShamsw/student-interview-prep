...
"""Persistence layer for Project 45: Interactive Unit Converter Plus Calculator."""

DATA_DIR = Path(__file__).resolve().parent / 'data'
OUTPUTS_DIR = DATA_DIR / 'outputs'
CASE_LIBRARY_FILE = DATA_DIR / 'conversion_sets.json'
RUN_CATALOG_FILE = DATA_DIR / 'runs.json'
HISTORY_FILE = DATA_DIR / 'calculator_history.json'
...
def load_case_library() -> List[Dict[str, Any]]:
    """Load reusable conversion cases from disk."""
    ensure_data_dirs()
    return _load_list_file(CASE_LIBRARY_FILE)


def save_case_library(library: List[Dict[str, Any]]) -> str:
    """Persist conversion cases to disk."""
    ensure_data_dirs()
    _save_list_file(CASE_LIBRARY_FILE, library)
    return str(CASE_LIBRARY_FILE)
...
def load_history() -> List[Dict[str, Any]]:
    """Load calculator history entries from disk."""
    ensure_data_dirs()
    return _load_list_file(HISTORY_FILE)


def save_history(history: List[Dict[str, Any]]) -> str:
    """Persist calculator history entries to disk."""
    ensure_data_dirs()
...
def save_trace_file(trace_payload: Dict[str, Any], session_id: str) -> str:
    """Persist converter trace payload for a session."""
    ensure_data_dirs()
    ...
    catalog.append(
        {
            ...
            'conversions_run': run_record.get('conversions_run', 0),
            'expressions_run': run_record.get('expressions_run', 0),
            'runtime_points': run_record.get('runtime_points', 0),
            'elapsed_ms': run_record.get('elapsed_ms', 0.0),
            'fastest_series': run_record.get('metrics', {}).get('fastest_series', ''),

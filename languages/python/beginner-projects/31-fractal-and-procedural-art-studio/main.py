"""
main.py - Entry point for Project 31: Fractal And Procedural Art Studio
"""

from display import format_header, format_run_report, format_startup_guide
from models import create_studio_config
from operations import load_studio_profile, run_core_flow
from storage import ensure_data_dirs


def main() -> None:
    """Run one complete fractal/procedural art generation session."""
    ensure_data_dirs()
    print(format_header())

    config = create_studio_config()
    profile = load_studio_profile()
    print(format_startup_guide(config, profile))

    summary = run_core_flow()
    print(format_run_report(summary))


if __name__ == '__main__':
    main()
